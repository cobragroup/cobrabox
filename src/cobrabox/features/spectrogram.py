...
def spectrogram(
    ...
    *,
    ...
    Args:
        data: Data with ``space`` and ``time`` dimensions.  Additional
            dimensions are supported and preserved in the output.
    ...
    Example:
        ...
        >>> sg = cb.feature.spectrogram(data)
        >>> sg.data.dims
    ...
    if scaling not in _VALID_SCALINGS:
        raise ValueError(f"scaling must be one of {_VALID_SCALINGS!r}, got {scaling!r}")

    n_time = xr_data.sizes["time"]
    seg = min(256, n_time) if nperseg is None else nperseg

    if seg < 2:
        raise ValueError("nperseg must be at least 2")
    if seg > n_time:
        raise ValueError(f"nperseg ({seg}) must not exceed the number of time samples ({n_time})")
    if noverlap is not None and noverlap >= seg:
        raise ValueError(f"noverlap ({noverlap}) must be less than nperseg ({seg})")
    ...
    scipy_scaling = "density" if scaling in ("log", "density") else "spectrum"
    if scaling == "magnitude":
        f_axis, t_axis, _ = _sp_stft(
            arr_flat[0], fs=fs, nperseg=seg, noverlap=noverlap, window=window
        )
    else:
        f_axis, t_axis, _ = _sp_spectrogram(
            arr_flat[0], fs=fs, nperseg=seg, noverlap=noverlap, window=window, scaling=scipy_scaling
        )
    ...
    for i in range(n_total):
        sig = arr_flat[i]
        if scaling == "magnitude":
            _, _, Zxx = _sp_stft(sig, fs=fs, nperseg=seg, noverlap=noverlap, window=window)
            out_flat[i] = np.abs(Zxx)
        else:
            _, _, Sxx = _sp_spectrogram(
                sig, fs=fs, nperseg=seg, noverlap=noverlap, window=window, scaling=scipy_scaling
            )
            if scaling == "log":
