...
def _energy_ratio(signal: np.ndarray, fs: float, window_samples: int) -> np.ndarray:
...
def _page_hinkley(er: np.ndarray, v: float, lam: float) -> int | None:
...
def epileptogenicity_index(
    ...
    *,
    ...
    Args:
        data: SEEG/EEG ``Data`` spanning the seizure period (baseline + ictal). Must
            have exactly ``time`` and ``space`` dimensions. ``sampling_rate`` must be
            set.
    ...
    Returns:
        ...
        rapid discharge is detected in any channel, all values are 0. The ``@feature``
        wrapper adds a singleton ``time`` dimension, giving final dims
        ``(space, time=1)``.
    ...
    Example:
        >>> ei = cb.feature.epileptogenicity_index(seizure_data)
        >>> ei_vals = ei.data.squeeze("time")   # (space,) values in [0, 1]
    """
    ...
    window_samples = max(2, round(window_duration * fs))
    h_samples = max(1, round(integration_window * fs))
    ...
    for ch in range(n_channels):
        er = _energy_ratio(arr[ch], fs, window_samples)
        nd = _page_hinkley(er, bias, threshold)
        er_list.append(er)
    ...
    for i in range(n_channels):
        ...
        ei_values[i] = er_slice.sum() / (nd - n0 + tau)
