...
def _mean_squared_coherence(x: np.ndarray, y: np.ndarray, nperseg: int) -> np.ndarray:
...
@feature
def coherence(data: Data, *, nperseg: int | None = None) -> xr.DataArray:
    """Compute magnitude-squared coherence for all pairwise channel
    combinations.
    ...
    Args:
        data: Data with 'space' and 'time' dimensions.  Additional dimensions
            are supported.
        nperseg: Samples per FFT segment.  Defaults to ``min(256, n_time)``.
            Must be >= 2 and <= n_time.

    Returns:
        xarray DataArray with dims ``(*extra_dims, 'space', 'space_to')`` where
        both ``space`` and ``space_to`` are indexed by the original channel
        coordinates.  Values are in [0, 1]; the diagonal is NaN.

    Raises:
        ValueError: If data has fewer than 2 spatial channels or ``nperseg``
            is invalid.
    ...
    for i, j in itertools.combinations(range(n_space), 2):
        ...
        y = arr[..., j, :]
        val = _mean_squared_coherence(x, y, nperseg=seg)
        coh_matrix[..., i, j] = val
