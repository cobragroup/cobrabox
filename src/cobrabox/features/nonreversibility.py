"""Non-reversibility feature: deviation from causal normality (dc_norm)."""
...
def _compute_dc_norm(X: np.ndarray) -> float:
    ...
    Returns:
        Scalar dc_norm value in [0, inf). Returns 0.0 if A + B^T is zero.
    ...
    # Forward:  X_t  = A @ X_{t-1}
    A = _fit_var1(X)
    # Reverse:  X_t  = B @ X_{t+1}  ↔  forward fit on time-reversed signal
    B = _fit_var1(X[:, ::-1])

    A = _rescale_to_unit_spectral_radius(A)
    B = _rescale_to_unit_spectral_radius(B)
...
class Nonreversibility(BaseFeature[SignalData]):
    """Compute dc_norm: deviation from causal normality (time-irreversibility).
    ...
    Complete reversal of causality occurs when A^T = B (i.e. A is a normal
    matrix: A^T A = A A^T). The dc_norm metric quantifies deviation from this
    symmetry after rescaling both matrices so their largest eigenvalue
    magnitude is less than 1:

        dc_norm = ||A - B^T||_F / ||A + B^T||_F

    A dc_norm of 0 indicates perfect time-reversibility; larger values
    indicate stronger asymmetry (irreversibility).
