...
@dataclass
class RecurrenceMatrix(BaseFeature[SignalData]):
    ...
    **3-D input** ``(N, N, T)`` — time-series of FC matrices:
    ...
    rec_metric: RecMetric = field(default="cosine")
    fc_options: list[str | int | float] = field(default_factory=list)

    # Derived fields unpacked from fc_options in __post_init__
    _fc_metric: FcMetric = field(default="pearson", init=False, repr=False)
    ...
    def __post_init__(self) -> None:
        if self.rec_metric not in get_args(RecMetric):
            raise ValueError(
                f"rec_metric must be one of {get_args(RecMetric)}, got '{self.rec_metric}'"
            )

        opts = self.fc_options
        if len(opts) > 3:
            raise ValueError(
                "fc_options may have at most 3 elements: [fc_metric, window_size, overlap]"
            )

        if opts:
            fc_metric_str = str(opts[0])
            if fc_metric_str not in get_args(FcMetric):
                raise ValueError(
                    f"fc_options[0] (fc_metric) must be one of {get_args(FcMetric)},"
                    f" got '{fc_metric_str}'"
                )
            self._fc_metric = cast(FcMetric, fc_metric_str)
    ...
    @staticmethod
    def _similarity_matrix(vecs: np.ndarray, metric: RecMetric) -> np.ndarray:
    ...
    @staticmethod
    def _fc_matrix(window: np.ndarray, fc_metric: FcMetric) -> np.ndarray:
        ...
        match fc_metric:
            ...
            case "MI":
                ...
                for i in range(n_ch):
                    ...
                    for j in range(i, n_ch):
                        xj = np.digitize(
                            window[:, j], np.histogram_bin_edges(window[:, j], bins=bins)
                        )
            ...
            case "PLV":
                ...
                phases = _analytic_signal_1d(
                    window.T, representation="phase", axis=1
                )  # (n_channels, n_time)
            ...
            case "AEC":
                # AEC — wrap window into SignalData and use EnvelopeCorrelation feature
    ...
    def __call__(self, data: SignalData) -> xr.DataArray:
        ...
        if n_spatial not in (1, 2):
            raise ValueError(f"Expected 1 or 2 spatial dimensions, got {n_spatial}: {spatial_dims}")
        ...
        if n_spatial == 2:
            ...
            if N1 != N2:
                raise ValueError(
                    f"For 3-D input the two spatial dimensions must be equal, got {N1} x {N2}"
                )
            triu_idx = np.triu_indices(N1, k=1)
            flat_vecs = np.array([arr[:, :, t][triu_idx] for t in range(n_time)])
            mat = self._similarity_matrix(flat_vecs, self.rec_metric)
            n = n_time
            ...
            if not self.fc_options:
                ...
                mat = self._similarity_matrix(X, self.rec_metric)
                n = n_time
                ...
                if ws < 5:
                    warnings.warn(
                        f"window_size={ws} is very small (<5); FC estimates may be unreliable.",
                        UserWarning,
                        stacklevel=2,
                    )
                if ws >= n_time:
                    raise ValueError(f"window_size ({ws}) must be < n_time ({n_time})")
                step_size = max(1, int(ws * (1.0 - self._overlap)))
                triu_idx = np.triu_indices(n_channels, k=1)
                starts = range(0, n_time - ws + 1, step_size)
                flat_vecs = np.array(
                    [self._fc_matrix(X[s : s + ws], self._fc_metric)[triu_idx] for s in starts]
                )
                mat = self._similarity_matrix(flat_vecs, self.rec_metric)
                n = len(starts)
