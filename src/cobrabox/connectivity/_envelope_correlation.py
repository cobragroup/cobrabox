...
@functional(EnvelopeCorrelation)
def envelope_correlation(
    data: SignalData, orthogonalize: str | bool = "pairwise", absolute: bool = False
) -> Data:
    """Compute amplitude envelope correlation (AEC) between all channel pairs.
    ...
    Example:
        >>> aec = cb.envelope_correlation(data)
        >>> aec_signed = cb.envelope_correlation(data, absolute=False)
        >>> aec_no_orth = cb.envelope_correlation(data, orthogonalize=False)
    """
    return EnvelopeCorrelation(orthogonalize=orthogonalize, absolute=absolute).apply(data)
