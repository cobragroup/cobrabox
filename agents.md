...
**Feature system** (`src/cobrabox/base_feature.py`, `src/cobrabox/feature.py`, and the domain packages under `src/cobrabox/`): Features are `@dataclass` classes that inherit one of three base classes:
...
Feature discovery is automatic: `feature.py` scans every subdirectory of `src/cobrabox/` except `egg/` and `_*`, importing each `.py` file (excluding only `__init__.py`) and registering any callable with `_is_cobrabox_feature = True` **and** `__module__ == <that module>` (the `__module__` filter prevents base classes imported into feature files from being registered as duplicates). Adding a new feature means creating a new **private** file in `src/cobrabox/<domain>/` — `_my_feature.py` — with a class inheriting the appropriate base, then running `scripts/gen_functional_wrappers.py` to seed its one-shot function. Implementation modules are private (the scipy/scikit-learn convention) so the lower-case name stays free for the functional API; `tests/test_public_api.py` fails if a feature lands in a public module. Domain packages: `windowing/`, `surrogates/`, `transforms/`, `signalstats/`, `infometrics/`, `spectral/`, `connectivity/`, `decompositions/`. Each domain's `__init__.py` re-exports its feature classes and their one-shot functions, and `src/cobrabox/__init__.py` re-exports every domain's `__all__`. A feature is therefore reachable as `cb.<Feature>` / `cb.<feature>` (canonical), `cb.<domain>.<Feature>` (domain-specific), or `cb.feature.<Feature>` (flat, auto-discovered). Every feature also carries a `_tags: ClassVar[list[str]]`; `scripts/gen_feature_docs.py` turns the registry + tags into the domain, tag, and API doc pages.

**Datasets** (`src/cobrabox/datasets.py`, `src/cobrabox/dataset_loader.py`, `src/cobrabox/dataset.py`): `cb.load_dataset(name)` returns a `Dataset[SignalData]`. `Dataset[T]` is a generic, immutable sequence of `Data` objects (`src/cobrabox/dataset.py`) with helpers: `.filter(subjectID=..., groupID=..., condition=...)` → `Dataset[T]`, `.groupby("groupID")` → `dict[str, Dataset[T]]`, `.describe()` prints a summary. Supports indexing, slicing, iteration, and `+` for concatenation. Built-in dummy datasets (`dummy_chain`, `dummy_random`, `dummy_star`, `dummy_noise`) are loaded from compressed CSV files in `data/dummy/`. The datasets differ in structure:
...
**Public API** (`src/cobrabox/__init__.py`): Top-level imports expose `Data`, `SignalData`, `EEG`, `FMRI`, `dataset`, `from_numpy`, `from_xarray`, the base classes `BaseFeature`, `SplitterFeature`, `AggregatorFeature`, `Pipeline`, and `Chord`, the domain subpackages, and **every feature** in both forms — the class `cb.LineLength` and the one-shot function `cb.line_length` (GH #116). The root re-export block is written by hand, one `from .<domain> import (...)` per domain; `tests/test_public_api.py` and `tests/test_functional_api.py` fail if it drifts from what discovery finds. Top-level implementation modules (`data`, `dataset`, `datasets`, `downloader`, …) are dropped from the namespace, so import them as `from cobrabox import downloader`, never `cb.downloader`. The `feature` submodule is also accessible as `cb.feature.*` (auto-discovered), and every domain as `cb.<domain>.*`. **Note:** `globals().update()` in `feature.py` is opaque to IDEs/type-checkers; the `# noqa: PLE0604` there is intentional. See `docs/plans/2026-02-27-feature-autodiscovery-static-analysis.md` for the open decision on a permanent fix.

**Feature D&D Alignments**: Every feature has a D&D alignment capturing its "moral character" — how it treats your data. Law axis: +1 = Lawful (imposes structure), 0 = Neutral (passive), -1 = Chaotic (disruptive). Good axis: +1 = Good (preserves meaning), 0 = Neutral (indifferent), -1 = Evil (discards). Use `uv run python -m cobrabox.egg.dnd_alignment --roster` to see the full grid, or `uv run python -m cobrabox.egg.dnd_alignment F1 F2 F3` to compute a pipeline's aggregate alignment.
...
- Feature classes live in `src/cobrabox/<domain>/` as individual **private** files, one class per file: `_line_length.py` defines `LineLength`. Never name the file after the public symbol without the underscore.
...
- Test files for features live under `tests/features/<domain>/` and are named `test_feature_<feature_name>.py` (e.g. `line_length.py` → `tests/features/signalstats/test_feature_line_length.py`).
- Every feature also gets a one-shot function (`cb.line_length(d)`) defined **in the same file as its class**, just below it, decorated with `@functional(TheClass)` (scikit-learn's layout). `scripts/gen_functional_wrappers.py` seeds it from the class — non-destructively, so it never clobbers a tuned docstring — and is NOT part of pre-commit. The class stays the composable form; aggregators are class-only. Because the wrapper is real source, `cb.line_length?` points at the feature file and type-checkers see its signature.
- `src/cobrabox/_dummy.py` is a negative reference (no useful docstring, no validation) — do not model new features after it.
- Top-level implementation modules (`data`, `dataset`, `datasets`, `downloader`, `base_feature`, `dataset_loader`, `egg`) are dropped from the `cobrabox` namespace in `__init__.py`. Import them as `from cobrabox import downloader`, not `cb.downloader`; `monkeypatch.setattr` must take the module object, not a dotted string.
