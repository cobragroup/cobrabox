...
Feature discovery is automatic: `feature.py` scans every subdirectory of `src/cobrabox/` except `egg/` and `_*`, importing each `.py` file (excluding only `__init__.py`) and registering any callable with `_is_cobrabox_feature = True` **and** `__module__ == <that module>` (the `__module__` filter prevents base classes imported into feature files from being registered as duplicates). Adding a new feature means creating a new **private** file in `src/cobrabox/<domain>/` — `_my_feature.py` — with a class inheriting the appropriate base, then re-exporting it from that domain's `__init__.py`. Implementation modules are private (the scipy/scikit-learn convention) so the lower-case name stays free for the functional API; `tests/test_public_api.py` fails if a feature lands in a public module. Domain packages: `windowing/`, `surrogates/`, `transforms/`, `signalstats/`, `infometrics/`, `spectral/`, `connectivity/`, `decompositions/`. Each domain's `__init__.py` re-exports its feature classes and installs the generated functions, and `src/cobrabox/__init__.py` re-exports every domain's `__all__`. A feature is therefore reachable as `cb.<Feature>` / `cb.<feature>` (canonical), `cb.<domain>.<Feature>` (domain-specific), or `cb.feature.<Feature>` (flat, auto-discovered). Every feature also carries a `_tags: ClassVar[list[str]]`; `scripts/gen_feature_docs.py` turns the registry + tags into the domain, tag, and API doc pages.
...
- Every feature also gets a one-shot function (`cb.line_length(d)`) generated from its dataclass by `src/cobrabox/_functional.py`; the class stays the composable form. Aggregators are class-only. Docstring `Example:` blocks on a feature class show the class form — the function's docstring is composed separately.
