"""Tests for the OutwardStrength feature."""
...
def test_returns_vector() -> None:
    """OutwardStrength produces a (space,) vector."""
...
def test_with_frequency_band() -> None:
    ...
    result = cb.OutwardStrength(freq_band=(20.0, 60.0)).apply(mat)
    assert result.data.dims == ("space",)
    assert result.data.shape == (4,)
...
def test_matches_manual_computation() -> None:
    """OutwardStrength matches manual column-mean (excluding diagonal)."""
    ...
    # outward[j] = mean of column j excluding diagonal
    expected = np.array(
        [
            np.mean([0.4, 0.7]),  # col 0
            np.mean([0.2, 0.8]),  # col 1
            np.mean([0.3, 0.6]),  # col 2
        ]
    )
    np.testing.assert_allclose(result.to_numpy(), expected, atol=1e-10)


def test_inward_outward_difference_equals_rc() -> None:
    """InwardStrength - OutwardStrength should equal ReciprocalConnectivity."""
    mat = _asymmetric_matrix(n=4)
    inward = cb.InwardStrength().apply(mat)
    outward = cb.OutwardStrength().apply(mat)
    rc = cb.ReciprocalConnectivity(freq_band=None).apply(mat)
    np.testing.assert_allclose(inward.to_numpy() - outward.to_numpy(), rc.to_numpy(), atol=1e-10)


def test_space_coords_preserved() -> None:
    """Space coordinates from space_from are used in output."""
...
def test_history_appended() -> None:
    ...
    result = cb.OutwardStrength().apply(mat)
    assert result.history[-1] == "OutwardStrength"


def test_does_not_mutate_input() -> None:
    mat = _asymmetric_matrix(n=3)
    original = mat.data.values.copy()
    _ = cb.OutwardStrength().apply(mat)
...
def test_pipes_after_directed_connectivity() -> None:
    """PartialDirectedCoherence | OutwardStrength end-to-end."""
    rng = np.random.default_rng(0)
    sig = cb.SignalData.from_numpy(
        rng.standard_normal((200, 3)), dims=["time", "space"], sampling_rate=128.0
    )
    pipeline = cb.PartialDirectedCoherence() | cb.OutwardStrength(freq_band=(0.0, 30.0))
