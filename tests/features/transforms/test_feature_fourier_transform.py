...
def test_forward_transform_default_magnitude() -> None:
    ...
    result = cb.feature.FourierTransform().apply(data)
...
def test_forward_transform_complex_coefficients() -> None:
    rng = np.random.default_rng(0)
    data = _signal(rng.standard_normal((128, 3)))
    result = cb.feature.FourierTransform(return_magnitude=False).apply(data)
...
def test_requires_time_dim() -> None:
    ...
    with pytest.raises(ValueError, match="time"):
        cb.feature.FourierTransform().apply(bad)
...
def test_roundtrip_with_inverse() -> None:
    ...
    data = _signal(arr)
    forward = cb.feature.FourierTransform(return_magnitude=False).apply(data)
    back = cb.feature.InverseFourierTransform(n=128, sampling_rate=200.0).apply(forward)
...
def test_inverse_requires_frequency_dim() -> None:
    ...
    with pytest.raises(ValueError, match="frequency"):
        cb.feature.InverseFourierTransform().apply(bad)


def test_inverse_requires_complex_input() -> None:
    rng = np.random.default_rng(0)
    data = _signal(rng.standard_normal((64, 2)))
    forward_mag = cb.feature.FourierTransform(return_magnitude=True).apply(data)
    with pytest.raises(ValueError, match="complex"):
        cb.feature.InverseFourierTransform().apply(forward_mag)
...
def test_inverse_default_length_doubles_minus_one() -> None:
    ...
    data = _signal(arr)
    forward = cb.feature.FourierTransform(return_magnitude=False).apply(data)
    back = cb.feature.InverseFourierTransform().apply(forward)
...
def test_history_appended() -> None:
    rng = np.random.default_rng(0)
    data = _signal(rng.standard_normal((64, 2)))
    forward = cb.feature.FourierTransform().apply(data)
