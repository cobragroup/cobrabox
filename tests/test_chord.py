...
def test_splitter_pipe_feature_returns_chord_builder() -> None:
    builder = cb.SlidingWindow(window_size=4, step_size=2) | cb.LineLength()
    assert isinstance(builder, _ChordBuilder)


def test_chord_builder_pipe_feature_extends_pipeline() -> None:
    builder = cb.SlidingWindow(window_size=4, step_size=2) | cb.LineLength() | cb.Min(dim="time")
    assert isinstance(builder, _ChordBuilder)
    assert isinstance(builder.pipeline, cb.Pipeline)


def test_chord_builder_pipe_aggregator_returns_chord() -> None:
    chord = cb.SlidingWindow(window_size=4, step_size=2) | cb.LineLength() | cb.MeanAggregate()
...
def test_pipe_syntax_result_equals_explicit_chord() -> None:
    data = _make_data()
    via_pipe = (
        cb.SlidingWindow(window_size=4, step_size=2) | cb.LineLength() | cb.MeanAggregate()
    ).apply(data)
...
def test_incomplete_chord_builder_raises_on_apply() -> None:
    builder = cb.SlidingWindow() | cb.LineLength()
...
def test_splitter_pipe_directly_to_aggregator_raises() -> None:
    with pytest.raises(TypeError, match="at least one pipeline step"):
        cb.SlidingWindow() | cb.MeanAggregate()
...
def test_chord_multi_step_pipeline_via_pipe() -> None:
    data = _make_data()
    out = (
        cb.SlidingWindow(window_size=4, step_size=2)
        | cb.LineLength()
        | cb.Mean(dim="space")
        | cb.MeanAggregate()
    ).apply(data)
...
def test_mean_aggregate_values_match_manual_mean() -> None:
    ...
    windows = list(cb.SlidingWindow(window_size=4, step_size=2)(data))
    expected = np.mean([cb.LineLength().apply(w).to_numpy() for w in windows], axis=0)
...
def test_pipeline_or_extends_pipeline() -> None:
    ...
    p = cb.LineLength() | cb.Mean(dim="space")
    assert isinstance(p, Pipeline)
    p2 = p | cb.Min(dim="time")
